# 🛡️ Classes (modo infantil)

No núcleo infantil, a classe serve para dar **identidade** e **escolhas visíveis**.

Regra base:

- ❤️ Vida: **3 corações**
- 🛡️️ Armadura: **0 a 4 escudos (🟦)**
- 🎲 Toda rolagem: **1D6** (6 = algo legal)

Cada classe abaixo já vem com uma sugestão pronta de **arma + poder**.

## ⚔ Guerreiro

- 🛡️ Armadura inicial: 🟦🟦
- 🗡 Arma sugerida: **espada**, **machado** ou **martelo**
- ✨ Poder (escolha 1):
  - **Resistente**: 1 vez por combate, quando tirar **1**, trate como **2–3**
  - **Protetor**: 1 vez por combate, transforme dano de um aliado perto em **0**

## 🏹 Ranger

- 🛡️ Armadura inicial: 🟦
- 🗡 Arma sugerida: **arco** ou **lança**
- ✨ Poder (escolha 1):
  - **Ágil**: quando tirar **6** atacando à distância, você pode se mover e se esconder
  - **Caçador**: 1 vez por cena, você pode declarar que “já tinha visto pegadas” e ganhar uma pista clara

## 🔥 Mago

- 🛡️ Armadura inicial: 🟦🟦 (Armadura Arcana)
- 🔮 Elemento (escolha 1): 🔥 Fogo / ❄ Gelo / 🌪 Vento / 🌿 Natureza / ✨ Luz
- ✨ Poder:
  - **Magia do elemento**: você pode descrever qualquer magia do seu elemento; no **6**, além de funcionar, faz um efeito extra do elemento (empurra, prende, protege, cura, etc.)

## ✝ Clérigo

- 🛡️ Armadura inicial: 🟦
- ✨ Milagre (escolha 1):
  - **Cura**: em **4–5** cura **1**; em **6** cura **1** e ainda dá **Protegido**
  - **Proteção**: em **4–5** dá **Protegido**; em **6** protege e afasta um inimigo
  - **Comando**: em **4–5** o inimigo hesita; em **6** o inimigo perde a próxima ação

> Equipamentos e efeitos das armas ficam em `equipamentos.md`.

