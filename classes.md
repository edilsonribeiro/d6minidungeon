# 🛡️️️ Classes (modo infantil)

No núcleo infantil, a classe serve para dar **identidade** e **escolhas visíveis**.

Regras rápidas:

- 🎲 Toda rolagem: **1D6** (6 = algo legal)
- ❤️ Vida: **2 corações**
- 🛡️️️ Armadura: **escudos 🟦** (até 4)
- 🔵 Mana: **bolinhas 🔵** (por aventura; cada magia custa 1; **máx. 1 por cena**)

Fichas prontas para imprimir: `fichas/infantil/README.md`.

Recompensas prontas para escolher: `arvore-de-recompensas.md`.

## ⚔ Guerreiro

- ❤️ Vida inicial: **2**
- 🛡️️️ Armadura inicial: 🟦🟦🟦🟦
- 🔵 Mana: **1**
- 🗡️ Arma sugerida: Espada / Machado / Martelo
- ✨ Magia (escolha **1**) — cada uso custa **1 Mana**:
  - **Protetor**: redirecione **todo o dano** de um aliado perto para você (você pode apagar 🟦 normalmente)
  - **Resistente**: ignore um resultado **1–3** contra você (trate como **4–5**)
  - **Golpe poderoso**: após acertar um ataque, cause **+1 dano** (total 3) em um alvo

## 🏹 Ranger

- ❤️ Vida inicial: **2**
- 🛡️️️ Armadura inicial: 🟦🟦🟦
- 🔵 Mana: **2**
- 🗡️ Arma sugerida: Arco / Duas adagas / Lança
- ✨ Magias (escolha **2**) — cada uso custa **1 Mana**:
  - **Disparo preciso**: seu ataque não pode ser bloqueado pela armadura (a armadura do alvo não consegue bloquear esse dano)
  - **Movimento fantasma**: ataque e depois se mova livremente para longe/para cobertura, sem complicação
  - **Olhos do caçador**: converta um **2–3** em **4–5** (em ataque ou exploração)

## 🔥 Mago

- ❤️ Vida inicial: **2**
- 🛡️️️ Armadura inicial: 🟦 (escudo mágico)
- 🔵 Mana: **4**
- 🗡️ Foco mágico: Cajado / Varinha / Livro
- 🔮 Elemento (escolha 1): 🔥 Fogo / ❄ Gelo / 🌪 Vento / 🌿 Natureza / ✨ Luz
- ✨ Magias — cada uso custa **1 Mana** (escolha na hora):
  - **Magia elemental**: ataque mágico (dano normal do sistema)
  - **Efeito elemental**: empurre / prenda / congele / proteja / cure 1 (escolha 1)
  - **Explosão arcana**: cause **2 de dano** em uma área pequena (ou 2 alvos perto)
  - **Escudo arcano**: ganhe **Protegido** automaticamente

## ✝ Curandeiro

- ❤️ Vida inicial: **2**
- 🛡️️️ Armadura inicial: 🟦🟦
- 🔵 Mana: **3**
- 🗡️ Foco sagrado: Cajado / Símbolo / Maça
- ✨ Magias — cada uso custa **1 Mana** (escolha na hora):
  - **Cura**: cure **1** coração (no **6**, cure **2**)
  - **Proteção**: dê **Protegido** para um aliado
  - **Comando sagrado**: um inimigo hesita e perde a próxima ação

Limite recomendado:

- um personagem **não pode receber Cura duas vezes na mesma cena**

> Equipamentos e efeitos das armas ficam em `equipamentos.md`.
