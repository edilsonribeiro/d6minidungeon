# 🎲 Introdução — RPG D6 Heroico

O sistema usa apenas **D6** e é **rápido**: a mesma rolagem serve para tudo.  
Em combate, ele não é mais *player-facing*: **jogadores e monstros rolam** (ataque x defesa).

Este projeto tem um resumo em 1 página: `nucleo-infantil.md`.

## 🚀 Começo rápido (primeira sessão)

Para começar sem confundir:

- leia `nucleo-infantil.md`
- use `aventura-inicial.md`
- no fim, cada herói escolhe 1 recompensa em `arvore-de-recompensas.md`

## 🧒 Núcleo infantil (padrão)

O núcleo completo (regra principal + combate simples + recursos) está em: `nucleo-infantil.md`.

Para aprofundar por tema:

- recursos, Destino e magia improvisada: `recursos.md`
- combate, condições e módulo de grid: `combate.md`
- classes e habilidades: `classes.md`
- equipamentos (armas): `equipamentos.md`
- monstros (como narrar e montar ficha simples): `monstros.md`

## 💤 Descanso

- **Descanso curto**: sem recuperação automática (use só como “pausa na história”)
- **Descanso completo**: recupera **todos os recursos** (❤️/🛡️/🟣/🔵)

## 🎁 Evolução (recompensas)

Ao final de **cada aventura**, escolha **1 recompensa** na árvore: `arvore-de-recompensas.md`.

## 🎯 Filosofia do sistema

- Rápido
- Cinematográfico
- Decisões visíveis
- Fácil para criança
- Profundo para adulto

## 🧩 Recursos

Detalhes e valores por classe:

- `recursos.md`

## 📎 Apêndice — Modo heroico (quando a mesa crescer)

Este apêndice é para uma versão mais “tradicional”, com atributos. Ele **não é o padrão** do modo infantil.

Use quando as crianças já estiverem confortáveis com o núcleo (ou para jogar com adultos).

### 📊 Atributos

Todo personagem tem 4 atributos:

- 💪 **Força**
- 🏹 **Destreza**
- 🧠 **Mente**
- ❤️ **Espírito**

### 🎯 Criação de personagem

Distribua **8D6** entre os atributos.

Regras:

- Mínimo **1D6** por atributo
- Máximo **4D6** por atributo

Exemplos:

- 4 / 2 / 1 / 1
- 3 / 3 / 1 / 1
- 2 / 2 / 2 / 2

### 🎲 Rolagem (modo heroico)

1. Escolha o atributo apropriado.
2. Role todos os D6 daquele atributo.
3. Use **apenas o melhor resultado** (mesma tabela do núcleo).
